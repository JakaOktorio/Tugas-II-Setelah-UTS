...
$usrid = $_POST['usrid'];
if ($proc == "usrdel") {
    $query = "DELETE FROM data_user WHERE id=$usrid";
    ...
    //end for
    $query = "INSERT INTO data_user (username,pass,fullname,email,telp,baned,akses) "
            . "VALUE ('$username',PASSWORD('$pass'),"
            . "'$fullname','$email','$telp','$baned',$aks)";    
}elseif ($proc == "usredt" && $usrid > 0) {
    $query = "SELECT id,username,fullname,email,telp,baned,akses FROM data_user WHERE id=".$usrid;
    ...
    $email = $_POST['email'];
    $telp = $_POST['telp'];
    $query = "UPDATE data_user SET username='$username',fullname='$fullname',email='$email',telp='$telp' WHERE id=".$usrid;    
}
